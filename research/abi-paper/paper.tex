...
including zero-byte function aliases.
\end{abstract}

\keywords{calling conventions, register allocation, irregular architectures,
  Z80, compiler optimization, interprocedural optimization}
...
\begin{center}
\small
Nanz $\to$ Parser $\to$ HIR $\to$ \fbox{PFCCO} $\to$ MIR2 $\to$ PBQP~RA $\to$ Z80
\end{center}
%
PFCCO runs after HIR-to-MIR2 lowering (which assigns Layer~1 defaults) and
before the PBQP register allocator.
...
\begin{tabular}{@{}llll@{}}
\toprule
Work & Scope & Reg.\ file & Optimal? \\
\midrule
Krause '13      & Intra     & Irregular & Yes \\
Krause '22      & Global    & Irregular & Best-of-$N$ \\
De Bus '04      & Per-fn    & Regular   & Heuristic \\
LLVM IPRA       & Inter     & Regular   & Exact$^\dagger$ \\
Wall '86        & Inter     & Regular   & Heuristic \\
\textbf{PFCCO}  & \textbf{Per-fn} & \textbf{Irregular} & \textbf{Optimal} \\
\bottomrule
\multicolumn{4}{@{}l}{\footnotesize $^\dagger$For save/restore only, not class assignment.}
\end{tabular}
\end{table}
...
demonstrated its effectiveness in a production compiler for the Z80.
