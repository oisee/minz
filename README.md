...
- **[The Nanz Language: A Complete Guide](docs/Nanz_Language_Book.md)** — Comprehensive book on Nanz: syntax, type system, HIR→MIR2→Z80 pipeline, iterator chains, LUTGen, struct methods/UFCS/interfaces. **New chapters (2026-03-10):** Chapter 9 — `@extern` with register class annotations (`z80_a`/`z80_hl`/etc.), RST-with-DB pattern analysis and recommended asm-wrapper approach; Chapter 10 — complete CondRetSink + CmpSubCarry optimization walk-through with T-state tables, u8 vs u16 comparison, and inline survival analysis.
