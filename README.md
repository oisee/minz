...
- **[All 23 Showcase Examples Passing: ForRange & Codegen Fixes](reports/2026-03-12-061-All_Showcase_Passing_ForRange_And_Codegen_Fixes.md)** — Four bugs fixed in one session, closing the last failing showcase example (`ex14_fold_assert`). **(1)** `lowerForRange` precomputes the loop upper bound once before entering the while loop — the `n+1` ADD instruction was inside the loop header, forcing `n` into A where any A-clobbering operation (loads, mul16 counter) corrupted it on every iteration. **(2)** u16 global loads via `LD HL,(sym)` (20T, no A clobber) instead of the HL-pointer path that used `LD A,(HL)` — eliminated the loop-counter clobber in `factorial`. **(3)** Constant u8 global stores via `LD HL,sym / LD (HL),imm` — the previous `LD A,0 / LD (sym),A` clobbered the function parameter (n in A per ABI). **(4)** `OpSub` HL↔DE: `EX DE,HL` is a true swap — `emitMov("HL","DE")` does byte-by-byte copy, not a swap; `abs_diff_u16` was computing `b−b=0` instead of `b−a`. **23/23 PASS.**

- **[Codegen Correctness Fixes & Allocator Architecture Roadmap](reports/2026-03-12-060-Codegen_Fixes_And_Allocator_Roadmap.md)** — Three Z80 codegen bugs fixed: **(1)** u8→u16 zero-extension in all store paths (`LD (IX+1), D` → `LD (IX+1), 0`) — storing a u8 value as u16 was writing the byte twice; **(2)** 16-bit self-pointer load HL←(HL) via A-scratch (`LD A,(HL) / INC HL / LD H,(HL) / LD L,A`) — the INC/DEC trick corrupted the address when `dst == ptr`; **(3)** `mul16` operand order and 8-bit zero-extension — rhs was loaded into DE *after* `PUSH BC / LD B,H / LD C,L` (which destroyed rhs-in-C), and 8-bit rhs wasn't zero-extended (writing the byte into both D and E). Net result: **+3 showcase examples passing** (ex2_ufcs, ex3_iface, ex9b_factorial_fold), **0 regressions**, **20/23 showcase total**. Architecture section reviews 3 directly relevant compiler papers from `/Volumes/safe/z80-compiler/` (Krause 2022 on Z80 calling conventions — validates our register-first ABI; Krause 2020 LOSPRE — linear-time redundancy elimination with register pressure awareness; Krause 2024 Padauk C compiler — pseudo-register patterns) and maps out the path from current greedy allocator → **Phase 6e PBQP edge costs** (~150 LOC, handles correlated allocation: LUT BC★, mul16 rhs class, DJNZ B propagation) → **lospre** (~500 LOC, reduces live range length → fewer spills) → **EXX save regions** (borrows shadow bank in hot loops).

- **[Frontend & Backend Diagnostic — Complete System Health Report](reports/2026-03-11-059-Frontend_Backend_Diagnostic.md)** — Honest state of every frontend, backend, and tool in the compiler system as of v0.19.5. **25/28 test packages green.** Active pipeline (Nanz/PL/M → HIR → MIR2 → Z80) fully healthy with 160+ MIR2 tests passing. 3 red packages: `pkg/codegen` (vet warnings in experimental 6502/GB/M68K backends — trivial fix), `cmd/repl` (5-minute type cast fix, semantically broken anyway), `tests/` (2 old-MinZ tests that test Nanz-only features — should be moved). All 6 open bugs documented. Includes per-backend status table: Z80/MIR2 (production ✅), MIR2→C (correctness oracle ✅), MIR2→QBE (modern targets ✅), QBE→MIR2 (parser ✅), old multi-backend (experimental, vet-blocked 🔴).

- **[`range(lo..hi)` iterator source + emitParallelCopy cycle fix](reports/2026-03-11-057-Range_Iterator_And_Parallel_Copy_Fix.md)** — Two bugs fixed, one new feature shipped. **(1)** `range(lo..hi)` is now a first-class counter-based iterator source: `range(0..n).fold(0, |acc,i| acc+i)` compiles to **1 instruction per iteration** — `ADD A, B / DJNZ`, zero spills, zero loads. No memory pointer; the DJNZ counter IS the element. **(2)** `PropagateConstants` infinite loop on DJNZ loops fixed (`BodyArgs[i]` → `incoming[{body, i+1}]` — the counter param is implicit, BodyArgs map to Params[1:]). **(3)** `emitParallelCopy` 8-bit A↔B cycle fix — when A is in the cycle, the algorithm wrongly used A as scratch, overwriting it before the final restore. Now picks the first 8-bit register not in the cycle (C, E, H, L, D) as scratch: `LD C, A / LD A, B / LD B, C` instead of the destructive `LD A, B / LD B, A`. MZE-verified: sum_range(0..10) all correct. Design discussion: dual-VM asserts (`@sandbox(mir2, z80)`) that run on both VMs by default and distinguish algorithm bugs (both wrong) from codegen bugs (MIR2 right, Z80 diverges).
...
- **[The Nanz Language Book v3](docs/Nanz_Language_Book_v3.md)** ⭐ **NEW** — Complete English rewrite. 11 chapters + 3 appendices covering everything in the v0.19.5 compiler: full HIR→MIR2→Z80 pipeline, PBQP register allocator, dual-VM compile-time assertions (runs on both MIR2 VM *and* assembled Z80 binary via emulator), LUT synthesis, BranchEquiv, CondRetSink+flag fusion, multiply strength reduction (`*256` → `LD H,L / LD L,0`, 7× faster), iterator chain fusion, UFCS, zero-cost interfaces, `range(lo..hi)` counter iteration, MIR2→C (correctness oracle) and MIR2→QBE (modern target) backends with rationale, and a verified codegen showcase with real assembly output. Supersedes v2.
- **[The Nanz Language Book v2](docs/Nanz_Language_Book_v2.md)** — Previous version. 13 chapters + 5 appendices.
  - Translations: [Русский](docs/Nanz_Language_Book_v2_RU.md) · [Українська](docs/Nanz_Language_Book_v2_UK.md) · [Español](docs/Nanz_Language_Book_v2_ES.md) · [Slovenčina](docs/Nanz_Language_Book_v2_SK.md)
- **[Open Bugs & Root Cause Analysis](docs/Open_Bugs_RCA.md)** — 6 tracked MIR2/codegen bugs with full RCA: BUG-001 GCD parallel-copy bloat (PBQP affinity edges missing), BUG-002 forEach constant rematerialization, BUG-003 `ptr[i]` in while loop (invalid `EX DE,HL` / `ADD F,DE`), BUG-004 non-zero-lo LUT contract mismatch, BUG-005 `applySubSwapNeg` missing u16 guard, BUG-006 zero-size struct globals not emitted. Priority/severity/fix-size for each.
- **[Nanz Z80 Output Quality & Allocator Trilogy](reports/2026-03-11-054-Nanz_Z80_Output_Quality_And_Allocator_Trilogy.md)** — 823-line deep-dive covering three layers shipped in one session: **(1)** struct literal syntax `Color{r: 255, g: 0, b: 0}` with in-place construction (no alloca for assignment targets), enabling HL-chain optimization automatically; **(2)** `LD (HL), n` immediate store folding — constant fields now emit `LD (HL), 255` (10T/2B) instead of `LD C, 255; LD (HL), C` (14T/3B), saving 4T/1B per constant field store; **(3)** `StorageClass` taxonomy and ADR-0019 — three tiers of global storage: `StorageNormal` (DB data section), `StorageSMCGetter` (values baked as instruction immediates in getter function), `StoragePhantom` (per-read-site immediates), and `StorageBakedSprite` (ZX Spectrum compiled sprite — pixel data AND screen addresses baked as instruction immediates in synthesised draw function, 346T vs ~1344T LDIR for 16×8 sprite). Foundation for ZX Spectrum baked sprite codegen: `set_pos()` patches address immediates; `set_frame()` patches pixel immediates.
- **[Nanz Real ASM Showcase: `abs_diff` Optimal Z80 — 4 Instructions](reports/2026-03-10-051-Nanz_Real_ASM_Showcase.md)** — Full five-pass optimization chain that transforms `abs_diff(a,b:u8)` from 8 naive instructions to `SUB B / RET NC / NEG / RET` (4 instructions, ~19T fast path). Passes: CondRetSink (hoist trivial return), SubSwapNeg (sub(b,a)→neg(r)), HoistReorderSubBeforeCmp (Sub before Cmp), CmpSubCarry (carry already in F — no CP emitted), PBQP interference elimination (ClassAcc mismatch gone). Inline survival analysis: `TermCondRet` becomes `TermBrIf` → `JR NC, .label` at the call site — same 4 instructions, zero CALL/RET overhead. Also covers the u16 case: `AND A; SBC HL,DE; RET NC` fast path (30T) with 16-bit NEG via NEG+SBC sequence.
