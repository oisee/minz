...
- **[FAT Cache Fix + Multi-Cluster](reports/2026-03-18-091-FAT_Cache_Fix_MultiCluster.md)** — Fixed sector-relative FAT cache offsets (was using absolute byte offsets on 512B buffer). Multi-cluster write/read verified: 3000B file spans 2 clusters, FAT chain 2→3→EOC, gcc reads all bytes. 12 total FatFS tests.
- **[E2E Multi-Channel FatFS Verification](reports/2026-03-17-090-E2E_MultiChannel_FatFS_Verification.md)** — 5-channel cross-verification: Nanz writes FAT12 image (text, binary, multi-cluster, delete, overwrite) → verified by Nanz VM, fresh Nanz VM, gcc FatFS R0.16, C89 MIR2 VM, raw byte inspection. All channels PASS. 12 total FatFS tests.
