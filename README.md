...
- **[A Byte's Journey: Five Translations, One Answer (EN)](reports/2026-03-10-053-Round_Trip_Deep_Dive_EN.md)** / **[Жизнь байта: пять трансляций, один ответ (RU)](reports/2026-03-10-052-Round_Trip_Deep_Dive_RU.md)** — Full walk-through of every intermediate representation as `abs_diff` passes through five layers: MIR2 (typed SSA with Z80 register classes) → QBE IL (`u8` widened to `w`, classes erased) → MIR2 again (phi nodes reconstructed as block params, register names shuffled but CFG topology intact) → C99 (`uint16_t` instead of `uint8_t`) → native binary. Shows the real `m.Dump()` and QBE IL at each stage, documents the information loss table (type precision, `[acc]`/`[flag]` classes, module name) and what survives (CFG, block params, computation). Includes the `fib` self-referencing phi node, Z80 output with `RET CGT` conditional return. Explains why Path 4 (crazy double translation) is a precision bug locator: "paths 1 and 2 diverge → bug in `mir2c`; paths 2 and 4 diverge → bug in `qbe2mir2`". Four paths, one answer: `abs_diff(10, 3) = 7`.
