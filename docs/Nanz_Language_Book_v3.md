...
**Version:** MinZ compiler v0.19.5+ (commit 09478bc)
**Date:** 2026-03-11
**Status:** Core features stable and verified
...
### 3.3 The u32 "ClassDWord" via EXX Shadow Pair
...
The calling convention is **not fixed** — it is computed per function by the PBQP register allocator and interprocedural contract optimizer. Typical Z80 mapping:
...
The **contract optimizer** (Phase 5b) searches across the call graph for the assignment that minimizes total T-states for caller+callee. The result is that calling conventions are tailored to the specific set of functions in your program — not a fixed ABI.
...
| map+forEach | ✅ | ~50T/elem |
| filter+forEach | ✅ | ~52T/elem |
| map+filter+forEach | ✅ | ~57T/elem |
| take+forEach | ✅ | ~43T/elem |
| skip+forEach | ✅ | ~43T/elem |
| lambda map | ✅ | ~50T/elem |
| lambda filter | ✅ | ~52T/elem |
| multi-stage | ✅ | varies |
| fold | ✅ | ~30T/elem |
| forEach with capture | ✅ | ~30T/elem |
...
This means `range(0..n).fold(0, |acc,i| acc+i)` computes **sum(1..n) = n*(n+1)/2**, not sum(0..n-1). This is intentional: counting down is the natural direction for DJNZ, and computing the triangular number is the correct mathematical result for "sum all elements in the counter's range."
...
### 6.3 `range(n..0)` — Reverse Direction

```nanz
fun f(n: u8) -> u8 {
    return range(n..0).fold(0, |acc: u8, i: u8| { return acc + i })
}
```

`range(n..0)` sets `Rev=true` — the count is `n` directly, no subtraction needed. Same DJNZ loop, same body. No `SUB` instruction at any point.

### 6.4 `range(3..8)` — Constant Bounds

```nanz
fun f() -> u8 {
    return range(3..8).fold(0, |acc: u8, i: u8| { return acc + i })
}
```

The constant-folding pipeline reduces `Sub(8, 3)` to `Const(5)`. The DJNZ counter is initialized with `LD B, 5`. The loop runs exactly 5 times.
...
**Root cause:** The parallel copy resolver used register A as a scratch for cycle-breaking. For the `A↔B` cycle in the range fold trampoline:
1. Save A → A (skipped: `if src != A`)
2. Walk cycle: `LD A, B` — overwrites A
3. Final: `LD B, A` — restores B's new value, not A's original

Both A and B end up as 0. The counter never loads n. Every iteration adds 0.

**Fix:** Collect the full set of registers in the cycle (`cycleRegs`). If A is in the cycle, pick the first non-cycle 8-bit register as scratch (C, E, H, L, D, B in order):
...
This bug was invisible to the MIR2 VM. The dual-VM assertion system (`assert sum_range(5) == 15`) would have caught it automatically.
...
No accumulator → no A↔B cycle → simple `LD B, A` trampoline.
...
Compile-time assertions are checked as part of compilation. If an assertion fails, the build fails. No separate test runner needed for basic correctness checks.
...
The check runs in two phases within the pipeline:
...
The A↔B parallel copy bug described in §6.5 is the canonical example of the third row. The MIR2 VM is ABI-agnostic — it doesn't simulate physical registers, so it cannot observe the swap corruption. The Z80 binary check catches it immediately.
...
Guideline:
- Use the default (both) for all new functions — it catches the most bugs.
- Use `via mir2` for very complex functions where Z80 execution is slow and you trust the codegen.
- Use `via z80` when debugging a known codegen issue.

### 7.5 How Z80 Asserts Work

The Z80 assert runner uses the **actual register allocation** — not a guess — to build the calling bootstrap:
...
The `@sandbox` annotation groups assertions and specifies which VMs to run them on. Individual asserts within a block can still override with `via z80` or `via mir2`. Planned for a future release.
...
ContractOpt     ← interprocedural calling convention optimization
...
**Result:** Replace `BrIf(CmpEq, @then, @else)` → `Jmp(@else)`. Save 10T + 3 bytes per call.

### 8.4 CondRetSink and Flag Fusion
...
**Pass 2: SubSwapNeg** — when the then-block has `b - a` and we already have `a - b`, replace with `NEG`:
...
**Pass 3: hoistReorderSubBeforeCmp** — moves the `Sub(a,b)` before the `Cmp(a,b)` so PBQP can see that the carry flag from `SUB` already contains the comparison result.

**Pass 4: CmpSubCarry** — replaces `Cmp(a,b)` with a no-op (carry already set by `SUB`). Condition code becomes `"C"` (carry).

**Pass 5: PBQP allocation** — with no interference between `a` and the `sub` result, both map to A. No spill register needed.
...
Total: 4 bytes per branch. **~22-35T** depending on which branch is taken. Compare with naive: 8 instructions, ~28-40T.

### 8.5 Multiply Strength Reduction

The `genMul16` function in Z80 codegen handles constant 16-bit multiplication with hardware-aware special cases.

**Standard power-of-2**: N × `ADD HL, HL`.

**Byte-boundary optimization (Fix A):** `* 256` does not need 8 × `ADD HL, HL` (56T, 16B). It is simply "move the low byte to the high byte and clear the low byte":

```asm
; x * 256:
LD H, L     ; H = L (low byte becomes high byte)
LD L, 0     ; L = 0
; result: HL = x * 256 in 8T, 2 bytes
```

For `* 512`, `* 1024`, etc. — byte-swap + remaining shifts:
...
LD L, 0
ADD HL, HL  ; × 2
; 19T, 5 bytes vs 63T, 18 bytes for 9×ADD HL,HL
```

**Small composites**: 3, 5, 6, 9 via PUSH HL / POP BC + shift + add sequences (no loop).

**General constant via decomposition** (planned Fix B): any constant with ≤3 set bits → shift+add decomposition. Constants with >3 set bits → Russian-peasant shift-and-add loop (current fallback, ~320T).

**Verification:** The mul16 codegen is verified with `TestMulU16ConstZ80` which runs every constant factor through the Z80 emulator:

```
x2: 3*2=6 ✓, 100*2=200 ✓
x256: 1*256=256 ✓, 200*256=51200 ✓  ← byte-swap path
x512: 1*512=512 ✓, 100*512=51200 ✓  ← byte-swap + 1 shift
```

### 8.6 Interprocedural Contract Optimization
...
Nanz/MIR2 supports multiple output backends. This is where things get interesting — and where MIR2→QBE and MIR2→C come in.
...
*mir2.Module
    ...
    ├── mir2qbe.Codegen    → .ssa (QBE)     [modern target testing]
    └── (planned) mir2llvm → LLVM IR        [x86/ARM native]
```

### 9.2 MIR2→C: Why We Have a C Backend

**MIR2→C is a verification and portability tool.** It translates MIR2 to a C99 source file that `gcc`/`clang` can compile and run.

**Why:**

1. **Cross-checking.** Run `abs_diff(10, 3)` on Z80 and on your host machine via C. If they disagree, you found a bug. This is the "triple-VM" idea: MIR2 VM, Z80 binary, and native binary via C all agree.

2. **Portability target.** A game or utility written in Nanz can be compiled to native code via C for development/testing — play-test the game logic on a fast machine before deploying to Z80.

3. **Language correctness.** The C backend uses standard arithmetic, which acts as a reference. If MIR2→Z80 gets `u8` overflow semantics wrong, `MIR2→C` won't.
...
Clean, readable, correct. Compiles with `gcc -O2 abs_diff.c` for native testing.

**Type mapping in mir2c:**
...
| u24, u32 | uint32_t |
| i24, i32 | int32_t |
| ptr | uint16_t (Z80 address width) |

### 9.3 MIR2→QBE: Why We Have a QBE Backend

**QBE** is a minimalist compiler backend by Quentin Carbonneaux that takes `.ssa` files and compiles them to x86-64, ARM64, or RISC-V native code. It is roughly "LLVM for minimalists."

**Why MIR2→QBE:**

1. **Modern target exploration.** QBE generates optimized native code for modern CPUs. The same Nanz function compiled via `MIR2→QBE→native` runs at full CPU speed. This lets us explore what Nanz programs *could* be on non-Z80 targets.

2. **IR validation.** QBE is strict about its SSA form. If `MIR2→QBE` produces QBE that QBE rejects, the MIR2 is malformed. It acts as a structural validator with different error messages than MIR2's own `Verify` pass.

3. **End-to-end testing of MIR2 semantics.** The `pkg/mir2qbe/e2e_test.go` tests compile Nanz to QBE, run through QBE → native, execute the result, and verify output. These E2E tests live alongside the Z80 E2E tests and catch semantic bugs in the MIR2 IR itself.
...
**QBE vs Z80:** QBE sees standard SSA. Z80 sees the same semantics but expressed in physical registers and flag conditions. The MIR2 IR bridges both — it is the shared representation that both backends consume.

**The big picture:** The same Nanz function, the same MIR2 IR, three verifiable targets:
...
If all four agree, you can be extremely confident the function is correct.

### 9.4 Z80 Targets

Within the Z80 backend, multiple platform targets are supported via assembler configuration:
...
When calling ROM routines that have fixed register requirements:
...
The `@z80_*` annotations override PBQP for those parameters, guaranteeing the right registers without wrappers.

### 10.4 The ABI Is Not Fixed

Unlike traditional languages, Nanz does not have a fixed calling convention. The contract optimizer assigns the best convention for each function based on usage. The result: calling conventions are smaller and faster than a fixed ABI.

You can observe the chosen convention in the generated assembly comment:
...
This chapter shows real Nanz programs and their verified Z80 output, demonstrating what "zero-cost abstraction" means in practice.
...
sum_range:
    ...
    JRS .rng_exit2
.rng_body1:
    ADD A, B         ← loop body: ONE instruction
    DJNZ .rng_body1
    LD B, A
    JP .rng_exit2
.rng_exit2:
...
All three asserts run on both VMs. All pass. ✅

### 11.3 Screen Address: Strength Reduction in Action

```nanz
fun pixel_addr(row: u8, col: u8) -> u16 {
    return u16(row) * 256 + u16(col)
}
```

```asm
; fun pixel_addr(row: u8 = A, col: u8 = B) -> u16 = HL
pixel_addr:
    LD H, A        ; H = row
    LD L, 0        ; L = 0  (byte-swap: row * 256)
    LD A, B
    LD L, A        ; add col
    RET
```
...
fun sin_approx(angle: u8<0..255>) -> u8 {
    // ... arbitrary computation, 50 instructions ...
}

assert sin_approx(0)   == 128   // midpoint
assert sin_approx(64)  == 255   // quarter-wave peak
assert sin_approx(128) == 128   // midpoint again
```
...
```asm
sin_approx_lut:
    DB 128, 131, 134, 137, ...   ; 256 bytes, precomputed by MIR2 VM

; fun sin_approx(angle: u8 = A) -> u8 = A
sin_approx:
    LD HL, sin_approx_lut
    LD D, 0
    LD E, A
    ADD HL, DE
    LD A, (HL)
...
```asm
; fun sum_filtered(buf: ^u8 = HL, n: u8 = B) -> u8 = A
sum_filtered:
    LD C, 0          ; total = 0
.loop:
    LD A, (HL)       ; load element
...
No `CALL` for filter, no `CALL` for forEach body, no intermediate buffer. `total` is threaded as register C throughout. Lambda capture at zero cost.
...
assert add32(0x00010000, 0x00010000) == 0x00020000
assert add32(0x0000FFFF, 0x00000001) == 0x00010000
```

```asm
; fun add32(a: u32 = HL/shadow, b: u32 = DE/shadow) -> u32 = HL/shadow
add32:
...
5 instructions. Uses the Z80 shadow register pair as the high half — perfectly legal Z80 assembly, and elegant.
...
fun_decl_inner = (op_sym | IDENT ('.' IDENT)?) '(' params ')' ('->' type)?
                 ('{' stmt* '}' | /* extern: no body */)
params         = (param (',' param)*)?
...
| F | ClassFlag | Carry = comparison result |
...
Each round-trip costs 26T. This is the "5× overhead" issue in complex loops. The PBQP affinity edge improvement (BUG-001) will reduce spills by guiding the allocator to prefer register-same assignments at loop back-edges.
...
```bash
# Compile with T-state annotations:
mz sum_range.nanz --annotate-tstates

# Output:
#   ADD A, B       ; 4T  ← loop body
#   DJNZ .rng_body1 ; 13T (taken) / 8T (not taken)
```
...
This edition documents features shipped since v2 (2026-03-09):

| Feature | Chapter | Status |
|---------|---------|--------|
| `range(lo..hi)` iterator source | 6 | ✅ shipped, E2E verified |
| Dual-VM compile-time asserts | 7 | ✅ shipped |
| `via mir2` / `via z80` assert syntax | 7 | ✅ shipped |
| `* 256` strength reduction | 8.5 | ✅ shipped, 7× speedup |
| Byte-boundary power-of-2 multiply | 8.5 | ✅ shipped |
| `emitParallelCopy` A↔B cycle fix | 6.5 | ✅ shipped |
| `PropagateConstants` DJNZ fix | 6.4 | ✅ shipped |
| MIR2→C backend | 9.2 | ✅ documented |
| MIR2→QBE backend | 9.3 | ✅ documented |
| u24/u32/i32 types | 3.1 | ✅ shipped |
| Fixed-point f8.8/f16.8/etc. types | 3.1 | ✅ shipped |
| ClassDWord (u32 via EXX shadow pair) | 3.3 | ✅ shipped |
| `^Struct` pointer receivers | 3.2 | ✅ shipped |
| Use-before-init warnings | 2.3 | ✅ shipped |
| Interface as parameter type (mono) | 4.3 | ✅ shipped |
| HL-chain store optimization | 2.7 | ✅ shipped |
| BranchEquiv pass | 8.3 | ✅ shipped |
| CondRetSink + CmpSubCarry | 8.4 | ✅ shipped |
| PBQP + post-allocation coalescing | 3.4 | ✅ shipped |
