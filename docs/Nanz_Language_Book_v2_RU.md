...
**Дата:** 2026-03-10
**Статус:** Активная разработка — основные функции стабильны, некоторые углы шероховаты
...
11. [PL/M-80: Посев корпуса и идиоматический перевод](#chapter-11-plm-80-corpus-seeding-and-idiomatic-translation)
...
Клауза `at(addr)` привязывает переменную к фиксированному адресу памяти. Чтение и запись становятся `LD (addr), r` / `LD r, (addr)`.
...
- **Неслитые лямбды** (указатели на функции): не могут захватывать внешние локальные переменные — нет кадра времени выполнения. Доступны только глобальные.
...
- Сбор рёбер сродства из `OpMove` и пар параметр блока↔аргумент.
...
## Глава 11: PL/M-80: Посев корпуса и идиоматический перевод

### 11.1 Роль PL/M в создании экосистемы Nanz

PL/M-80 послужил **загрузочным корпусом** для бэкенда MIR2. 26 файлов Intel 80 Tools (ALGOL-M, BASIC-80, макроассемблер ML80 и др.) — всё реальный производственный код 1970-х годов — предоставили 1 338 функций и 11 661 оператор для тестирования конвейера HIR→MIR2→Z80 ещё до появления Nanz.
...
### 11.3 Пробелы в функциональности (Nanz vs. MinZ)
...
### 11.4 Другие бэкенды (только MinZ)
