...
func Z80Codegen(m *Module, ar *AllocResult, opts ...Z80CodegenOptions) string {
	...
	// If a 'main' function exists and is not the first function, emit a JP to
	// it so the binary entry point (load address) always jumps to main.
	if m.FuncByName("main") != nil && len(m.Funcs) > 0 && m.Funcs[0].Name != "main" {
		sb.WriteString("    JP main\n\n")
	}
