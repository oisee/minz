...
func Compile(m *mir2.Module, opts CompileOptions) (string, error) {
	...
	// For Vulkan/Metal: only one entry point allowed. Emit first compilable
	// function as main(), skip the rest (or emit as helpers without results[]).
	for fi, f := range m.Funcs {
		name := gpuSym(f.Name)

		if opts.Backend == Vulkan {
			if fi > 0 {
				// Vulkan: emit helper as void function (no results[] access)
				sb.WriteString(fmt.Sprintf("// helper: %s (not a compute entry point)\n", name))
				continue
			}
			// Entry point
			...
			// CUDA/OpenCL/Metal: each function is a separate kernel
			sb.WriteString(spec.KernelDecl(name))
			sb.WriteString(" {\n")
			sb.WriteString("    " + spec.ThreadID + "\n")
		}
