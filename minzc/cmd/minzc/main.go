...
func init() {
	...
	rootCmd.Flags().BoolVar(&useLIR, "lir", false, "use LIR backend (ISLE+WFC+PBQP) for code generation")
	rootCmd.Flags().BoolVar(&useVIR, "vir", true, "use VIR backend (Z3 joint isel+regalloc, -71% vs SDCC, requires z3 in PATH) (default: on)")
